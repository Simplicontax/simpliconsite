...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      // GoDaddy accepts an UNSEEN search but crashes the Edge worker on SINCE/date searches.
      // Imported replies are marked as seen, so unread UIDs are the reliable sync cursor.
      const unreadUids = await client.search({ seen: false }, { uid: true });
      const candidateUids = unreadUids.slice(0, 25);
